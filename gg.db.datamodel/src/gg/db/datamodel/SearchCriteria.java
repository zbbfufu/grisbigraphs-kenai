 ...
 * <LI>The boolean operator is 'or': if the search criteria contains a list of two keywords, and if a transaction contains one of them,
...
public class SearchCriteria implements Comparable<SearchCriteria> {
    ...
    /** Creates a new instance of SearchCriteria that doesn't filter on anything */
     ...
     */
    public SearchCriteria(Currency currency, List<Account> accounts, Period period, List<Category> categories, List<Payee> payees, String keywords, boolean includeTransferTransactions) {
    ...
    @Override
    public int compareTo(SearchCriteria searchCriteria) {
        if (searchCriteria == null) {
            throw new IllegalArgumentException("The parameter 'searchCriteria' is null");
        }

        return period.compareTo(searchCriteria.getPeriod());
    }
